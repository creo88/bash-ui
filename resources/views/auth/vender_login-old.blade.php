...
@section('auth-lang')
    <select name="language" id="language" class="btn btn-primary my-1 me-2"
        onchange="this.options[this.selectedIndex].value && (window.location = this.options[this.selectedIndex].value);">
        @foreach (App\Models\Utility::languages() as $language)
            <option @if ($lang == $language) selected @endif value="{{ route('vender.login.lang', $language) }}">
...
@section('content')
    ...
    {{ Form::open(['route' => 'vender.login', 'method' => 'post', 'id' => 'loginForm']) }}
    @csrf
    <div class="">
        ...
        <div class="row">
            <div class="col-sm-6">
                <a href="{{ route('login') }}"
                    class="btn-login btn btn-primary btn-block mt-2 text-white">{{ __('User Login') }}</a>

            </div>
            <div class="col-sm-6 text-end">
                <a href="{{ route('vender.login') }}"
                    class="btn-login btn btn-primary btn-block mt-2 text-white">{{ __('Vendor Login') }}</a>
            </div>
        </div>
