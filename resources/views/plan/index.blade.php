...
@section('content')
                ...
                <div class="card price-card price-1 wow animate__fadeInUp" data-wow-delay="0.2s"
                   ...
                   ">
                    <div class="card-body">
                            ...
                            <div class="flex-row-reverse p-0 m-0 d-flex">
                                <div class="action-btn bg-primary ms-2">
                                    <a title="{{ __('Edit Plan') }}" href="#"
                                        ...
                                        data-bs-title="{{ __('Edit Plan') }}" data-bs-toggle="tooltip"
                        ...
                        <!-- @can('edit plan')
        <div class="col-4">
                                                        <a title="{{ __('Edit Plan') }}" href="#" class="m-1 btn btn-primary btn-icon" data-url="{{ route('plans.edit', $plan->id) }}" data-ajax-popup="true" data-title="{{ __('Edit Plan') }}" data-toggle="tooltip" data-original-title="{{ __('Edit') }}">
    ...
    @endcan -->
                                                ...
                                                @if ($plan->id != \Auth::user()->plan)
                                                    @if ($plan->price > 0)
                                                        <a href="{{ route('stripe', \Illuminate\Support\Facades\Crypt::encrypt($plan->id)) }}"
                                                            data-bs-toggle="tooltip" class="btn btn-primary btn-icon btn-sm"
                                                            data-bs-placement="top" title=""
                                                            data-bs-original-title="Subscribe">{{ __('Subscribe') }} <i
                                                                class="m-1 fas fa-arrow-right"></i>
                                                        </a>
                                                    @else
                                                        <span class="mb-2">{{ __('Free Plan') }}</span>
                                                        {{-- <a href="#" class="m-1 btn btn-primary btn-icon">{{__('Free')}}</a> --}}
                                                    @endif
                                                @endif
                                            ...
                                            <div class="col-3">
                                                @if (\Auth::user()->requested_plan != $plan->id)
                                                    <a href="{{ route('send.request', [\Illuminate\Support\Facades\Crypt::encrypt($plan->id)]) }}"
                                                        class="btn btn-primary btn-icon btn-sm" data-title="Cancle Request"
                                                        data-bs-toggle="tooltip" data-bs-placement="top" title=""
                                                        data-bs-original-title="Cancle Request">
                                                        <span class="btn-inner--icon"><i class="fas fa-share"></i></span>
                                                    </a>
                                                @else
                                                    <a href="{{ route('request.cancel', \Auth::user()->id) }}"
                                                        class="btn btn-danger btn-icon btn-sm" data-title="Cancle Request"
                                                        data-bs-toggle="tooltip" data-bs-placement="top" title=""
                                                        data-bs-original-title="Cancle Request">
                        ...
                        {{-- @if (\Auth::user()->type == 'company' && \Auth::user()->plan == $plan->id)
                            <p class="text-white server-plan">
        ...
        <div class="col-md-3">
                                        <a href="#" class="btn-addnew-project"  data-ajax-popup="true" data-size="lg" data-title="{{ __('Create New Plan') }}" data-url="{{ route('plans.create') }}">
                                            ...
                                            <p class="text-center text-muted">{{ __('Click here to add New Plan') }}</p>
                                        </a>
                                    </div>
    @endcan -->
    </div>
